<template>
    <div>
      ...
      <div v-if="isLoginVisible">
        <input type="text" v-model="email" placeholder="Email" @focus="focusInput('email')" @blur="blurInput('email')" />
        <input type="password" v-model="password" placeholder="Password" @focus="focusInput('password')" @blur="blurInput('password')" />
        <button @click="handleLogin" :disabled="!isLoginFormValid">Login</button>
      </div>
      <div v-else>
        <input type="text" v-model="registerEmail" placeholder="Register Email" @focus="focusInput('registerEmail')" @blur="blurInput('registerEmail')" />
        <input type="password" v-model="registerPassword" placeholder="Register Password" @focus="focusInput('registerPassword')" @blur="blurInput('registerPassword')" />
        <input type="password" v-model="confirmPassword" placeholder="Confirm Password" @focus="focusInput('confirmPassword')" @blur="blurInput('confirmPassword')" />
        <button @click="handleRegister" :disabled="!isRegisterFormValid">Register</button>
      </div>
      <button @click="toggleCard">Toggle Login/Register</button>
    </div>
  </template>
...
import authService from '../../util/auth/authService';
  ...
  export default {
    ...
    methods: {
      ...
      handleLogin() {
        authService.tryLogin(this.email, this.password)
          .then(user => {
            this.$router.push('/');
          })
          .catch(error => {
      ...
      handleRegister() {
        authService.tryRegister(this.registerEmail, this.registerPassword)
          .then(() => {
            this.toggleCard();
          })
          .catch(err => {
